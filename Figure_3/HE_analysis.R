...
regional <- read.csv("~/Data/Additional/regional_immmune.csv", header = TRUE, check.names = FALSE) %>% as.data.frame()
...
p + stat_compare_means() + scale_fill_manual(values = c("deeppink3", "deepskyblue4")) + xlab("Microbial Load Class") +ylab("Immune Hotspots - Fi") +theme(axis.title.x = element_text(size = rel(2)), 
