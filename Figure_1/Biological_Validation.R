...
tmp_dat <- read.csv("~/Documents/Projects/Microbial_TRACERx_P1/Data/16S_seq/Meta_Data/association_meta_new.csv", header = TRUE) %>% as.data.frame()

for (i in rownames(full_dat)) {
  i <- i %>% as.character()
  tmp <- tmp_dat %>% filter(samples == i) %>% as.data.frame()
  emphy <- tmp$emphysema 
  pneu <- tmp$pneumonia 
  
  full_dat[i, "emphysema"] <- emphy
  full_dat[i, "pneumonia"] <- pneu
}

full_dat <- full_dat %>% as.data.frame()
full_dat <- full_dat %>% select(-copd)
full_dat$pathogenic_load <- (full_dat$pseudomonas + full_dat$klebsiella + full_dat$staphylococcus +full_dat$streptococcus)
full_dat <- full_dat %>% as.data.frame()
...
p <- ggplot(full_dat, aes(x = pneumonia, y = pathogenic_load)) + geom_boxplot()
p +  theme_classic() + stat_compare_means() + scale_fill_brewer(palette = "BuPu") + xlab("Emphysema") + ylab("Pathogenic Microbial laad")
